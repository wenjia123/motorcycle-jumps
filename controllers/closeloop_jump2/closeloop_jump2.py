...
driveVelocity =9.9774+2#12.4553#16.1#15.7#15.2697#7.6348
Udes = driveVelocity
...
if recordData:
    ...
    f = open('../scripts/jump2_closeloop_suspension_data_takeoffonly.txt','w')
...
while robot.step(timestep) != -1 and simtime<=12:
    ...
    if(firstLoop):
        ...
        firstLoop=False
    m = MC_params[4]+MC_params[7]+MC_params[9]+MC_params[11]
    # Read the sensors:
    ...
    tf = D/(Udes*cos(theta_t)) #flying time 
    ...
    #thetadot_des = 0
    #BLOCK 2
    T1 = Straight and (x<30.6)
    T2 = Straight and (x>=30.6)
    T3 = Takingoff and (x<31.75)
    T4  = Takingoff and (x>=31.75)
    T5 = Flying and (x<31.75+D)
    T6 = Flying and (x>=31.75+D)
    T7 = Landing and True 
    #BLOCK 3
    Straight = T1 
    Takingoff = T3 or T2
    ...
    print(U,x,Straight,Takingoff,Flying,Landing)
